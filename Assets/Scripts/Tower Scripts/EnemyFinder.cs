...
public class EnemyFinder : MonoBehaviour
{
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        {
            if (tower.ImmutableType != ElementTypes.Whimsical && tower.ImmutableType != ElementTypes.Order)
            {
                tower.EnemyInRange = collision.transform;
            }
            else if(!tower.MultiEnemiesInRange.Contains(collision.transform))
            {
                tower.MultiEnemiesInRange.Add(collision.transform);
            }
        }
    }
